...
Format: `add n/NAME p/PHONE_NUMBER e/EMAIL a/ADDRESS [t/TAG] r/ ROOM_NUMBER
from/START_DATE to/END_DATE`

A guest can only be assigned to one room. The RoomNumber must also be valid.
...
=== Adding a person: `addx` [red]#Coming in v2.0#
...
=== Checkout room: `checkin`

Checks in the room, which marks the room's current booking as checked-in.
Format: `checkin ROOM_NUMBER`
...
* `checkout 085` +
Checks out room 085 and the guest staying in it.

=== Deleting a guest : `delete` [red]#Coming in v2.0#
...
=== Editing a guest : `edit` [red]#Coming in v1.3#
...
=== Locating guests by name: `find` [red]#Coming in v1.3#
...
* `find -g r/085` +
Find guest currently staying in room 085
* `find -r 085` +
Find room 085
* `find -r g/John Smith` +
Find room for which guest(s) with "John" or "Smith" or both in their names, has made a booking
* `find -r double` +
Find all double rooms
* `find -r avail` +
Find all available rooms today
* `find -r avail from/09/12/18 to/10/12/18` +
Find rooms that are available from 09/12/18 10/12/18. Input dates must be in DD/MM/YY format.

=== Listing all guests : `list`

Shows the entire list of rooms, or guests, depending on the input flag +
Format: `list [flag]`
...
=== Selecting a guest : `select` [red]#Coming in v1.3#
...
[NOTE]
====
Undoable commands: those commands that modify the address book's content (`add`, `checkin`, `checkout`, `delete`, `edit` and `clear`).
====
...
Clears all guests from the application. +
Format: `clear`
...
=== Export log: `export` [red]#Coming in v1.3#

Exports a log of all history of commands into a text file +
Format: `export`

=== Viewing help : `help` [red]#Coming in v1.3#
...
* *Add a Guest and assign a Room* : `add`
* *Checkin* : `checkin`
* *Checkout* : `checkout`
* *Delete* : `delete INDEX`
