...
* A valid booking cannot clash with an existing booking. It must also have a
  start date from today onwards (not outdated).
...
When a booking is added and it starts on a future date, then it can only be
viewed by clicking on the relevant room. It can be found in the "All other
bookings" section.
// end::add[]
...
* [v2.0] You can create new accounts via the Concierge app.
****
...
[NOTE]
====
Undoable commands: those commands that modify Concierge's content (`add`, `checkin`, `checkout` and `clear`).
====
...
image::service pre clear.jpg[width="419"]

The command box aft kbd:[Alt] is pressed:

image::service post clear.jpg[width="371"]
...
image::add PREFIX_NAME.jpg[width="581"]
