...
determine if the `Room` is free to book. Besides, `Room` s are not expected
to have a large number of advanced bookings made.
...
the `Room` s. Nevertheless, for most uses of Concierge we expect `Guest` s to
 be aware of their `Room` s.
...
checked-in guests. This list does not retain any `Booking` information, as it
 is meant to help the user quickly look up the `Guest` 's particulars.
// end::add[]
...
The `login` feature allows hotel managers to tag every command entered with a
 specific user, creating a blame to help trace rogue commands. Managers can also
 control which receptionists have full access to Concierge.
...
in when executing commands which would mutate the data, such as `add`,
...
image::LogInCommand-LogInManager-classdiagram.png[width=400]
...
A SHA-256 hash was used in building up this feature. In future, this hashing
algorithm can be changed to a HMAC hash which adds a username salt as well.
Then, different users will not know if they have chosen the same passwords.
...
** Pros: Manager can implement some level of access control within Concierge.
 Some of the more commonly used Concierge features (`list`, `find`) are
 read-only features.
 This is quicker than mandating a sign-in at the start and creating different user views
 based on the account privilege (admin vs normal).
...
While, `concierge.xml` is constantly being written to, this is an unnecessary feature
for the passwords component.
*** Furthermore, creating a new password entry is difficult since once has to add all
