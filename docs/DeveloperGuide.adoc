...
. `ModelManager#checkinRoom` (which implements Model) will call `VersionedAddressBook#checkinRoom`
. `VersionedAddressBook#checkinRoom` will call `UniqueRoomList#checkinRoom`
...
The Guest is added to the AddressBook, which was already the case in
AddressBook4.
