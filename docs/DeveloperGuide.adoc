...
[NOTE]
Note how the `Model` simply raises a `ConciergeChangedEvent` when the Concierge data are changed, instead of asking the `Storage` to save the updates to the hard disk.
...
* stores the Concierge data.
* exposes an unmodifiable `ObservableList<Person>` that can be 'observed' e.g. the UI can be bound to this list so that the UI automatically updates when the data in the list change.
* does not depend on any of the other three components.

[NOTE]
As a more OOP model, we can store a `Tag` list in `Concierge`, which `Person` can reference. This would allow `Concierge` to only require one `Tag` object per unique `Tag`, instead of each `Person` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
...
* can save the Concierge data in xml format and read it back.
...
It extends `Concierge` with an undo/redo history, stored internally as a `conciergeStateList` and `currentStatePointer`.
...
Step 2. The user executes `delete 5` command to delete the 5th guest in the concierge. The `delete` command calls `Model#commitConcierge()`, causing the modified state of the concierge after the `delete 5` command executes to be saved in the `conciergeStateList`, and the `currentStatePointer` is shifted to the newly inserted concierge state.
...
[NOTE]
If a command fails its execution, it will not call `Model#commitConcierge()`, so the concierge state will not be saved into the `conciergeStateList`.

Step 4. The user now decides that adding the guest was a mistake, and decides to undo that action by executing the `undo` command. The `undo` command will call `Model#undoConcierge()`, which will shift the `currentStatePointer` once to the left, pointing it to the previous concierge state, and restores the concierge to that state.
...
The `redo` command does the opposite -- it calls `Model#redoConcierge()`, which shifts the `currentStatePointer` once to the right, pointing to the previously undone state, and restores the concierge to that state.
...
Step 5. The user then decides to execute the command `list`. Commands that do not modify the concierge, such as `list`, will usually not call `Model#commitConcierge()`, `Model#undoConcierge()` or `Model#redoConcierge()`. Thus, the `conciergeStateList` remains unchanged.
...
* **Alternative 1 (current choice):** Maintain two columns on the MainWindow UI to display both rooms and guests
...
* **Alternative 2 :** Separately display the two lists within the same MainWindow UI space/column.
...
We current accept a `Guest`, `RoomNumber` and `BookingPeriod` as parameters
for the AddCommand constructor.

The Guest is added to the Concierge, which was already the case in
Concierge4.

We then create a new `Booking` object with the `Guest` and `BookingPeriod`.
This `Booking` is then added to the `Room` with the `RoomNumber` specified.

We do not record the `Room` details in the `Guest` to minimise coupling.
...
and `ExpenseType` have been created for this purpose. In addition, the hotel also has a menu of
...
the menu at any time. The `Menu` object is internally represented with a `HashMap<String, ExpenseType>`,
...
*Scenario:* You are in charge of `model`. One day, the `logic`-in-charge approaches you for help. He wants to implement a command such that the user is able to remove a particular tag from everyone in the concierge, but the model API does not support such a functionality at the moment. Your job is to implement an API method, so that your teammate can use your API to implement his command.

[TIP]
Do take a look at <<Design-Model>> before attempting to modify the `Model` component.

. Add a `removeTag(Tag)` method. The specified tag will be removed from everyone in the concierge.
+
****
* Hints
...
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/address/model/Concierge.java[`Concierge`] and link:{repoURL}/src/main/java/seedu/address/model/person/Guest.java[`Person`] classes can be used to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/address/model/Concierge.java[`Concierge`] allows you to update a guest, and link:{repoURL}/src/main/java/seedu/address/model/person/Guest.java[`Person`] allows you to update the tags.
* Solution
...
. Modify the link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] to show the total number of people in the concierge.
+
**Before**
+
...
** link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] needs to initialize the status bar on application start, and to update it accordingly whenever the concierge is updated.
* Solution
...
*Scenario:* You are in charge of `storage`. For your next project milestone, your team plans to implement a new feature of saving the concierge to the cloud. However, the current implementation of the application constantly saves the concierge after the execution of each command, which is not ideal if the user is working on limited internet connection. Your team decided that the application should instead save the changes to a temporary local backup file first, and only upload to the cloud after the user closes the application. Your job is to implement a backup API for the concierge storage.

[TIP]
Do take a look at <<Design-Storage>> before attempting to modify the `Storage` component.

. Add a new method `backupConcierge(ReadOnlyConcierge)`, so that the concierge can be saved in a fixed temporary location.
+
****
* Hint
...
We have to properly encapsulate the remark in our link:{repoURL}/src/main/java/seedu/address/model/person/Guest.java[`Person`] class. Instead of just using a `String`, let's follow the conventional class structure that the codebase already uses by adding a `Remark` class.
...
Now we have the `Remark` class, we need to actually use it inside link:{repoURL}/src/main/java/seedu/address/model/person/Guest.java[`Person`].

**Main:**

. Add `getRemark()` in link:{repoURL}/src/main/java/seedu/address/model/person/Guest.java[`Person`].
...
. Fix `invalidAndValidPersonConcierge.xml`, `typicalPersonsConcierge.xml`, `validConcierge.xml` etc., such that the XML tests will not fail due to a missing `<remark>` element.

===== [Step 6b] Test: Add withRemark() for `PersonBuilder`
Since `Person` can now have a `Remark`, we should add a helper method to link:{repoURL}/src/test/java/seedu/address/testutil/PersonBuilder.java[`PersonBuilder`], so that users are able to create remarks when building a link:{repoURL}/src/main/java/seedu/address/model/person/Guest.java[`Person`].

**Tests:**

. Add a new method `withRemark()` for link:{repoURL}/src/test/java/seedu/address/testutil/PersonBuilder.java[`PersonBuilder`]. This method will create a new `Remark` for the guest that it is currently building.
